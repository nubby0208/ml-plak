<template>
    <div>
      <div class="bordered-container">
        <div class="flex">
          <div class="container-title not-printable"  @click="containerExpandedModulo = !containerExpandedModulo">
            <span class="container-toggler less" v-if="containerExpandedModulo">▲</span>
            <span class="container-toggler more" v-else>▼</span>
            Módulo
          </div>
        </div>
        <div class="bordered-section" :class="{'uncollapsed': !containerExpandedModulo}">
          <b-container
            class="seccion-modulo"
          >
            <b-row
              class="text-left"
            >
              ...
              >
                ...
                <b-row
                  v-if="mounted && moduloLineal"
                  align-v="center"
                  class="not-printable justify-content-center hover-line"
                >
                  <div class="column margin-top-10">
                    <span
                      class="medidas"
              ...
              >
                ...
                <div v-if="!isAmbienteSelected" class="hover-line flex">
                  ...
                  <div class="column flex-1 align-center">
                    <select
                      id="material"
                      v-model="getMaterialDefaultPorModuloGrupo"
                      :title="getMaterialDefaultPorModuloGrupo"
                      @change="onMaterialChangeGrupo1($event)" 
                      name="material"
                      class="form-control form-control-sm"
                    >
                        <option
              ...
              >
                ...
                >
                  <b-col
                    cols="4"
                  >
                    <div class="not-printable justify-center">
                      <b-form-checkbox
                        v-model="zeroWidth"
                      >
                        Sin espesor
                  ...
                  >
                    <b-button
                      :disabled="!canClone"
                      size="sm"
                      class="not-printable btn-block"
                      :variant="canClone ? 'outline-primary' : ''"
                      @click="cloneModule"
                    >
                      Clonar Módulo                          
                    </b-button>
          ...
          <div class="seccion-acciones not-printable">
            <div class="flex justify-center">
              <div class="flex" v-if="!mounted && moduloLineal">
                <button class="btn btn-sm button__h-auto" v-on:click="setupHorizontal" :disabled="configureDisabled()">
                  Configurar módulo (Piso/Techo)
                </button>
                <button class="btn btn-sm button__h-auto" v-on:click="setupVertical" :disabled="configureDisabled()">
              ...
              <div class="flex" v-if="mounted">
                <button class="btn btn-sm button__h-auto" v-on:click="resetModule" v-if="!dividing">
                  Restablecer módulo
                </button>
              </div>
              ...
              <div class="flex">
                ...
                <button @click="saveElementConfig()" v-if="mounted" type="button" class="btn btn-primary btn-sm button__h-auto">Guardar Módulo</button>
                <button @click="loadElementConfig()" type="button" class="btn btn-primary btn-sm button__h-auto">Cargar Módulo</button> <span  class="v-separator" v-if="mounted">|</span>
      ...
      <div class="bordered-container">
        <div class="flex">
          <div class="container-title not-printable"  @click="containerExpandedAvanzado = !containerExpandedAvanzado">
            <span class="container-toggler less" v-if="containerExpandedAvanzado">▲</span>
            <span class="container-toggler more" v-else>▼</span>
            Avanzado
          </div>
        </div>
        <div class="avanzado-bordered-section bordered-section not-printable" :class="{'uncollapsed': !containerExpandedAvanzado}">
      ...
      <div class="bordered-container" v-if="mounted && !hasCustomFondos && materialFondo && !isAmbienteSelected">
        <div class="flex">
          <div class="container-title not-printable" @click="containerExpandedFondos = !containerExpandedFondos">
            <span class="container-toggler less" v-if="containerExpandedFondos">▲</span>
            <span class="container-toggler more" v-else>▼</span>
            Fondo
          </div>
        </div>
        <div class="bordered-section" :class="{'uncollapsed': !containerExpandedFondos }">
      ...
      <!-- Editar Fondo -->
      <EditCustomFondo v-if="selectedModule.customFondos.length" />
...
import EditCustomFondo from './EditCustomFondo'
...
export default {
  ...
  mounted () {
    ...
    // this.materialGrupo = this.getMaterialDefaultPorModuloGrupo
  ...
  watch: {
    ...
    selectedModule: function (newVal, oldVal) {
      ...
      // const materialGrupo = this.getMaterialDefaultPorModuloGrupo(this.selectedModule.moduleId)
      // this.materialGrupo = materialGrupo ? JSON.parse(materialGrupo) : ''

      const tapacanto = this.getTapacantosDefaultPorModulo(this.selectedModule.moduleId)
      // fix perdida de tapacanto
      let tcId = tapacanto ? JSON.parse(tapacanto).id : -1
      let tc = this.availableTapacantos.find(tc => tc.id === tcId)
      this.tapacanto = tc || ''
    }
  },
  computed: {
    getMaterialDefaultPorModuloGrupo: {
      get(){
        var id = this.selectedModule.moduleId;
        var material_default_por_modulo_grupo = this.$store.state.general[this.isAmbienteSelected ? 'material_default_por_modulo_room_editor' : 'material_default_por_modulo_grupo'][id - 1];
        if(material_default_por_modulo_grupo && JSON.parse(material_default_por_modulo_grupo) && JSON.parse(material_default_por_modulo_grupo) !== "" && JSON.parse(material_default_por_modulo_grupo) !== " " && JSON.parse(material_default_por_modulo_grupo) !== null){
          // alert(material_default_por_modulo_grupo);
          return JSON.parse(material_default_por_modulo_grupo);
        }else{
          this.$store.commit('setGeneralProperty', { key: 'material_default_grupo', value: JSON.stringify(this.material) })
          this.$store.commit('setDefaultMaterialForModuleGrupo', { material: JSON.parse(JSON.stringify(this.material)), module: this.selectedModule.moduleId, ambienteEnabled: this.isAmbienteSelected })
          return JSON.parse(this.$store.state.general[this.isAmbienteSelected ? 'material_default_por_modulo_room_editor' : 'material_default_por_modulo_grupo'][id - 1])
        }
      },
      set(val){
        // console.log(val);
        this.$store.commit('setGeneralProperty', { key: 'material_default_grupo', value: JSON.stringify(val) })
        this.$store.commit('setDefaultMaterialForModuleGrupo', { material: JSON.parse(JSON.stringify(val)), module: this.selectedModule.moduleId, ambienteEnabled: this.isAmbienteSelected })
        // this.materialGrupo = val;
      }
    },
    moduleSettings () {
      return this.$store.getters.selectedModule.settings
    },
    containerExpandedFondos: {
      get () {
        return this.$store.state.projectSettings.collapsibles.containerExpanded.fondos
      },
      set (value) {
    ...
    containerExpandedAvanzado: {
      get () {
        return this.$store.state.projectSettings.collapsibles.containerExpanded.avanzado
      },
      set (value) {
        this.$store.commit('setProjectSettingsCollapsibles', {
          key: 'containerExpanded',
    ...
    containerExpandedModulo: {
      get () {
        return this.$store.state.projectSettings.collapsibles.containerExpanded.modulo
      },
      set (value) {
        this.$store.commit('setProjectSettingsCollapsibles', {
          key: 'containerExpanded',
  ...
  methods: {
    ...
    uploadModuleImage () {
      ...
      let modAbreviatura  = 'M' + (parseInt(this.$store.getters.selectedModuleId) + 1)     

      console.log(`uploadModuleImage storreID: ${ this.$store.getters.selectedModuleId}, Modulo ${modAbreviatura} *-*disenio/src/components/DesignDashboard/Tools/EditModule.vue`);
      // let result =  {folder:modAbreviatura, description:"", folderGeneral:false}   
    ...
    materialDefaultGrupo () {
      // const material = this.getMaterialDefaultPorModuloGrupo(this.selectedModule.moduleId)
      // return material ? JSON.parse(material) : ''
    },
    ...
    //   if(material_default_por_modulo_grupo && JSON.parse(material_default_por_modulo_grupo) && JSON.parse(material_default_por_modulo_grupo) !== "" && JSON.parse(material_default_por_modulo_grupo) !== " " && JSON.parse(material_default_por_modulo_grupo) !== null){
    //     // alert(material_default_por_modulo_grupo);
    //     return material_default_por_modulo_grupo;
    //   }else{
    //     this.$store.commit('setGeneralProperty', { key: 'material_default_grupo', value: JSON.stringify(this.material) })
    //     this.$store.commit('setDefaultMaterialForModuleGrupo', { material: JSON.parse(JSON.stringify(this.material)), module: this.selectedModule.moduleId, ambienteEnabled: this.isAmbienteSelected })
    ...
    onMaterialChangeGrupo1: function () {

      if (this.$store.getters.selectedModule.elements.length > 0) {
        this.$swal('Aplicar cambio para el modulo seleccionado o a todos los elementos con propiedad grupo?', {
          title: 'Cambiar el material por defecto a grupos de elementos',
          icon: 'warning',
          buttons: {
            default: {
              text: 'Aplicar solo a este modulo',
              value: 0
            },
            empty: {
              text: 'Aplicar a todos los elementos con propiedad grupo',
              value: 1
            }
          }
        }).then((result) => {
          this.$store.commit('setGeneralProperty', { key: 'material_default_grupo', value: JSON.stringify(this.getMaterialDefaultPorModuloGrupo) })
          if(result == 1){
            this.$store.commit('updatePiecesMaterialGrupo', { material: this.getMaterialDefaultPorModuloGrupo, update: 0, allGrupos:true })
            this.$store.commit('setDefaultMaterialForModuleGrupo', { material: this.getMaterialDefaultPorModuloGrupo, module: this.selectedModule.moduleId, ambienteEnabled: this.isAmbienteSelected, allGrupos:true })
          }

          if(result == 0){
            this.$store.commit('updatePiecesMaterialGrupo', { material: this.getMaterialDefaultPorModuloGrupo, update: 0 })
            this.$store.commit('setDefaultMaterialForModuleGrupo', { material: this.getMaterialDefaultPorModuloGrupo, module: this.selectedModule.moduleId, ambienteEnabled: this.isAmbienteSelected })
          }
          
        })
      } else {
        // this.$store.commit('setGeneralProperty', { key: 'material_default_grupo', value: JSON.stringify(this.materialGrupo) })
        // this.$store.commit('updatePiecesMaterialGrupo', { material: this.materialGrupo, update: 0 })
        // this.$store.commit('setDefaultMaterialForModuleGrupo', { material: this.materialGrupo, module: this.selectedModule.moduleId, ambienteEnabled: this.isAmbienteSelected })
      }
    },

...
<div>
  <div class="bordered-container">
    ...
    <div class="separador-bordered-section bordered-section not-printable" :class="{'uncollapsed': !containerExpanded}">
      ...
      <div class="extras-size flex">
        ...
        <div v-if="horizontalPositioning" class="ml-control flex align-items-center hover-line">
          <label v-b-tooltip.hover title="Medida desde techo" for="top">Arriba P.</label>
          <input id="top" class="vw-4" ref="desdeArribaRoot" v-model="topRoot" type="number">
        </div>
        <div v-if="horizontalPositioning" class="ml-control flex align-items-center hover-line">
          <label v-b-tooltip.hover title="Medida desde piso" for="bottom">Abajo P.</label>
          <input id="bottom" class="vw-4" ref="desdeAbajoRoot" v-model="bottomRoot" type="number" >
        </div>
        
        <div v-if="verticalPositioning" class="ml-control flex  hover-line align-items-center">
          <label v-b-tooltip.hover title="Medida desde Lat. Izquierdo" for="left">Izquierda P.</label>
          <input id="left" class="vw-4" ref="desdeIzquierdaRoot" v-model="leftRoot" type="number" >
        </div>
        <div v-if="verticalPositioning" class="ml-control flex align-items-center hover-line">
          <label v-b-tooltip.hover title="Medida desde Lat. Derecho" for="right">Derecha P.</label>
          <input id="right" ref="desdeDerechaRoot" class="vw-4" v-model="rightRoot" type="number">
        </div>
      </div>
...
import { noReload } from '@/modules/utils.ts'
export default {
  data () {
    return {
      fondos: [],
  ...
  methods: {
    ...
    addDoor (isCorrediza) {
      ...
      EventBus.$emit("addDoorEvent");
    },
    addDobleFondoManual () {
      this.$store.commit('addDobleFondo')
      EventBus.$emit("addDobleFondoManualEvent");
    },
    hasDoor () {
  ...
  computed: {
    containerExpanded: {
      get () {
        return this.$store.state.projectSettings.collapsibles.containerExpanded.separador
      },
      set (value) {
    ...
    leftRoot: {
      get () {
        return Math.round(this.$store.getters.getWidthRoot(this.selectedElement.id))
      },
      set (value) {
        console.log(value);
        this.timedUpdate(() => this.$store.commit('setSeparatorLeftRoot', Number(value)))
      }
    },
    rightRoot: {
      get () {
        return Math.round(this.$store.getters.getWidthRootD(this.selectedElement.id))
      },
      set (value) {
        this.timedUpdate(() => this.$store.commit('setSeparatorRightRoot', Number(value)))
      }
    },
    right: {
      get () {
    ...
    topRoot: {
      get () {
        return Math.round(this.$store.getters.getHeightRootA(this.selectedElement.id))
      },
      set (value) {
        this.timedUpdate(() => this.$store.commit('setSeparatorTopRoot', Number(value)))
      }
    },
    bottomRoot: {
      get () {
        return Math.round(this.$store.getters.getHeightRoot(this.selectedElement.id))
      },
      set (value) {
        this.timedUpdate(() => this.$store.commit('setSeparatorBottomRoot', Number(value)))
      }
    },
    bottom: {
      get () {

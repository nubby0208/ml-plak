<template>
  <div class="bordered-container" v-shortkey="['alt', 'w']" @shortkey="setFocus('cant')">
    <div class="flex">
      <div class="container-title not-printable"  @click="toggleContainer('zona')">
        <span class="container-toggler more" v-if="containerExpanded['zona']">▼</span>
        <span class="container-toggler less" v-if="!containerExpanded['zona']">▲</span>
        Zona
      </div>
    </div>
    <div class="zona-bordered-section bordered-section not-printable" :class="{'uncollapsed': containerExpanded['zona']}">
...
export default {
  ...
  mounted () {
    this.containerExpanded['zona'] = this.isContainerExpanded('zona')
  },
  methods: {
    ...
    exportList () {
      if (this.isAmbienteSelected && (!this.$store.getters.getModuloModules || this.$store.getters.getModuloModules[0].elements.length === 0)) {
        let self = this

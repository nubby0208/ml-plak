    ...
    <div class="col-sm-12 col-md-6 col-lg-6 mt-3">
      ...
      <b-modal id="modal-reporte" hide-header hide-footer size="lg">
        ...
        <div class="modal-footer">
          ...
          <button @click="$bvModal.hide('modal-reporte')" type="button" class="btn btn-default" data-dismiss="modal">Cerrar</button>
        </div>
      </b-modal>
    ...
    <div class="col-sm-12 col-md-6 col-lg-6 mt-3">
      ...
      <div id="reportes-listado" class="col-sm-12">
        <div class="table-responsive">
          ...
          <b-table :items="reportesFinalizados" striped responsive="sm"
            :fields="fields"
                      ...
                      <b-row class="">
                        <b-col>
                              <table class="table table-striped table-bordered">
                                ...
                                <tbody>
                                  <template v-for="dt in row">
                                    <tr v-for="item in dt.datat" :key="item.id">
                                      <td>
                                          {{item.encuesta.esPregunta ? 'Pregunta' : item.esTexto ? 'Campo de texto' : 'Calificación'}}
                                      </td>

                                       <td class="text-left">
                                          {{item.encuesta.nombre}}
                                      </td>
          ...
          <div class="bg-dark row text-left">
            <div class="col-md-4">
              <label class="text-light mt-3 text-left">{{reportesFinalizados.length}}</label>
            </div>
            <div class="col-md-8 mt-2 mb-2 ">
              <b-pagination style="margin-bottom:0px !important" 
              v-model="pagOptions.active" 
              :total-rows="reportesFinalizados.length"
              :per-page="pagOptions.total"
...
export default {
    ...
    methods:{
      ...
      async traerEncuestasFinal() {
        let responseOne
        try {
          responseOne = await EncuestaRespuestaService.getAll()
        } catch (error) {
          console.log('error ln 447', error)
          responseOne = {}
        }
        ...
        //       observaciones: "",
        //       proyecto: "Cama 4 cajones 2 zapateros",
        //       senia: 30000,

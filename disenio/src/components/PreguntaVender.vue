  ...
  <div>
    <div class="preguntas-exportar">
      ...
      >
        ...
        <b-tbody v-if="!isBusy">
          <template v-for="(pe, i) in preguntasVender">
            <b-tr v-bind:key="pe.id">
              ...
              <b-th>
                  <b-form-select v-if="pe.edit" v-model="pe.estatus" :options="optionsPreguntasVender"></b-form-select>
              ...
              <th>
                ...
                <b-button
                  ...
                  @click="eliminarPreguntaVender(i)"
                >
...
export default {
  ...
  methods: {
    async getPreguntas(){
      ...
      return HTTP.get("/api/configuracion/tipo/preguntaVender/all").then(a=>{
          ...
          if(a.data.configuraciones.length > 0){
            ...
            this.preguntasVender = a.data.configuraciones.map(function(abc) {
              return {
                id: abc.id,
            ...
            this.countPreguntas = this.preguntasVender.length;
          }else{
          }
      });
    ...
    guardarPregunta(i){
      this.countPreguntas++;
      this.preguntasVender[i].edit = false;
      if(this.preguntasVender[i].id == null){
        HTTP.post('/api/configuracion', {
            name:`Pregunta ${this.countPreguntas}`,
            type:"preguntaVender",
            values:this.preguntasVender[i],
        }).then(result => {
          this.getPreguntas();
        ...
        HTTP.put(`/api/configuracion/${this.preguntasVender[i].id}`, {
            type:"preguntaVender",
            values:this.preguntasVender[i],
        }).then((result) => {
      ...
      console.log(this.preguntasVender[i]);

    },
    agregarPreguntaVender(){
        this.preguntasVender.push( {
            id: null,
            pregunta: "",
            estatus: "activo",
            edit: true
    ...
    modoPreguntaVender(pregunta, opc){
        console.log(pregunta, opc);
        this.preguntasVender[pregunta].edit = opc;
    },
    eliminarPreguntaVender(pregunta){
      ...
      this.$swal("No podrá volver a acceder a dicha pregunta", {
        ...
        if (value === true) {
          ...
          HTTP.delete(`/api/configuracion/${this.preguntasVender[pregunta].id}`).then(
            (response) => {

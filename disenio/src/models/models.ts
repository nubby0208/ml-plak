...
export class CuadroCad {

    public geometria: string = "cuadro";
    public x: number = 0;
    public y: number = 0;
    public alto: number = 0;
    public ancho: number = 0;
    public rotacion: object = {
        x:0,
        y:0,
        z:0,
    };
...
export class CirculoCad {

    public geometria: string = "circulo";
    public x: number = 0;
    public y: number = 0;
...
export class LineaCad {

    public geometria: string = "linea";
    public x: number = 0;
    public y: number = 0;
...
export class Element {
    ...
    public conexionesDrawing?: any = {}; // dibujo personalizado para la calcomania (en cara opuesta y frontal)
    public grupoMaterial?: boolean = false; // Establece el grupo de materiales
    ...
    static getAbsoluteX(element: Element): number {
        const parent = Element.GetParent(element) as Cube
        ...
        if (parent.layout !== 'vertical') {
            if ((element as any).separator && (element as any).separadorPadre != undefined) {
                if ((element as any).separatorDivision) {
                    return Element.getAbsoluteX((element as any).separadorPadre);
                }
                return (element as Separator).position.x
            }
            return Element.getAbsoluteX(parent)
        }

        if ((element as any).separator) {
            return Element.getAbsoluteX(parent) + (element as Separator).position.x
        }

        const leftSibling = Element.GetBeforeSibling(element) as Separator
        if (leftSibling) {
            return Element.getAbsoluteX(leftSibling)
        }
    ...
    static getFullWidthLeft(element: Element){
        var elm = Element.GetX(element);
        var total = 0;
        var i = 0;
        return elm;
        console.log({total:total, elm:elm, i:i, element:element});
        // while (elm || i == 10){
        //     i++;
        //     total += Element.GetWidth(elm);
        //     elm = Element.GetParent(elm);
        //     console.log({total:total, elm:elm, i:i, element:element, layout:elm.layout});
        // }
    ...
    static getAbsoluteY(element: Element): number {
        const parent = Element.GetParent(element) as Cube
        ...
        if (parent.layout !== 'horizontal') {
            return Element.getAbsoluteY(parent)
        }

        if ((element as any).separator) {
            return Element.getAbsoluteY(parent) + (element as Separator).position.y
        }

        const bottomSibling = Element.GetAfterSibling(element) as Separator
        if (bottomSibling) {
            return Element.getAbsoluteY(bottomSibling)
        }
        return Element.getAbsoluteY(parent)
    }
    ...
    static GetCalcoInfo(sep: Separator): CalcoInfo[] {
        ...
        if (separator.separator) {
            ...
            if (separator.drawing && (separator as any).drawing.front) {
                ...
                if((separator as any).conexionesDrawing && (separator as any).conexionesDrawing.front){
                    calcoInfo.conexionesDrawingCurrent = (separator as any).conexionesDrawing.front;
                }
            }
        ...
        if (separator &&
            ...
            calco2.conexionesDrawingCurrent = undefined;
            ...
            if (separator.drawing && separator.drawing.back) {
                ...
                if(separator.conexionesDrawing && separator.conexionesDrawing.back){
                    calco2.conexionesDrawingCurrent = separator.conexionesDrawing.back;
                }
            }
        ...
        if (separator.puerta) {
            ...
            if (puertaInfo.tipo == PuertaTipo.Dividida) {
                ...
                const tcDerecho = (calcoPuertaDer as any).separator['puerta'].tapacantos.derecho;
                const tcIzquierdo = (calcoPuertaDer as any).separator['puerta'].tapacantos.izquierdo;
                (calcoPuertaDer as any).separator['puerta'].tapacantos.izquierdo = tcDerecho;
                (calcoPuertaDer as any).separator['puerta'].tapacantos.derecho = tcIzquierdo;
        ...
        if (separator.dobleFondo) {
            ...
            calcoDobleFondo.conexionesDrawingCurrent = separator.dobleFondo.conexionesDrawing && separator.dobleFondo.conexionesDrawing.front;
...
export class Cajon extends Element {
    public frenteVisible  = true
    public baseVisible  = true
    public latIzqVisible  = true
    public latDerVisible  = true
    public cfVisible  = true
    public cf2Visible  = true
...
export class GeneralConfiguration {
    ...
    listLineasPremium:any = []
    listLineasClasica:any = []
    moduleGroups: any = []; // grupos de modulos que se activan o desactivan simultaneamente
}
...
export class Puerta {
    ...
    public puertasVisible: Array<boolean>
...
export class Modulo {
    ...
    conexionesDrawing?: any; // dibujo para la calco del fondo
    ...
    static getGrupoModules(): Modulo[] {
        ...
        return Element.state.modules.filter((m: Modulo) => m);
    }
...
export class GeneralInformation {
    ...
    fechaMedicion: any[] = []
    horaMedicion: any[] = []
    otrasFechaInstalacion: any[] = []
    otrasHoraInstalacion: any[] = []
    fechaInstalacion = ''
    horaInstalacion = ''
    comentarioInstalacion = ''
    comentarioResumenPresupuestoMaterial = ''
    comentario = ''
    saldo?= ''
    pagos?: any[] = []
    items?: any[] = [] // items del valor total
    links: string[] = [] // links externos
    linksDescripcion: string[] = [] // links externos descripcion
    seniaDescripcion?= ''
    totalDescripcion?= ''
...
export class CalcoInfo {
    ...
    conexionesDrawing?: any; // dibujo hecho con canvas
    conexionesDrawingCurrent?: any; // dibujo hecho con canvas
}
